...
<#include "/template/security/common/c_thickbox.ftl">
			...
			$(document).ready(function(){
				grid = new SimpleGrid({
					...
					columns : [
						...
						{name : "outId", title : "学号", width : "150px"},
						//{name : "appName", title : "应用名称", width : "150px"},
						{name : "userid", title : "玩校人员 id", width : "150px"},
						{name : "customerCode", title : "学校代码", width : "150px"},
						{name : "customerName", title : "学校名称", width : "150px"},
						{name : "state", title : "绑卡状态", width : "150px"},
						//{name : "param", title : "个人值", width : "400px"},
						{name : "date", title : "创建时间", width : "200px"}
					],
					multiple : true,
					isPagination : true,
					data : ${datas},
				...
				$("#student_view").click(function(){
					...
					if(checkboxkeys.length==0){
						...
						thickbox.attr("href","${ctx}/student/view.${urlExt}?id="+checkboxkeys.toString()+"&width=480&height=350");
						thickbox.click();
					}
				});
    			...
    			function changeToolbarState() {
					...
					$("#student_view").attr("disabled", !(hasSelected && (!selectedMany)));
					$("#student_del").attr("disabled", !(hasSelected && (!selectedMany)));
		    		}
	...
	<div class="main" id="main">
		...
		<div class="toolbar">
			<li><input type="button" id="student_view" disabled="disabled" class="view" style="display: none" value="查看明细"/></li>
			 	<li><input type="button" id="student_del" disabled="disabled" class="del" style="display: none" value="删除"/></li>
  		</div>
	</div>
    <div id="queryD" style="width:100%;">
				<div class="query">
					<form id="searForm" method="post" action="${ctx}/student/list.${urlExt}">
						<input type="hidden" name="currPage" id="currPage" value="1" />
						<input type="hidden" name="pageSize" id="pageSize" value="10" />
						<ul>
			        		<li>学号：</li>
			        		<li>
			          			<input type="text" id="outId" name="outId"/>
			        		</li>
			        		<li>学校代码：</li>
			        		<li>
			        		...
			        		<li>
			         			<input type="button" id="searchButton" value="查询" class="ButtonStyle" />
			        		</li>
			        	</ul>
			    	</form>
			    </div>
