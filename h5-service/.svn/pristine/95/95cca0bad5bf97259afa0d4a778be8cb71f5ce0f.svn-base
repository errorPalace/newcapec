...
<script language="javascript" src="${wctx}/pub/script/jquery/jquery.form.js"></script>
		<script language="javascript">
			$(document).ready(function(){
				grid = new SimpleGrid({
					...
					columns : [
                        {name : "sonActivityId", title : "举办活动", width : "80px"},
                        {name : "prizeId", title : "奖品", width : "80px"},
                        {name : "allotCount", title : "预计发奖数量", width : "80px"},
                        {name : "surplusCount", title : "上期叠加", width : "80px"},
                        {name : "inCount", title : "中奖数量", width : "80px"},
				...
				//删除
				$("#sonActivityPrizeCfg_del").click(function(){
					var checkboxkeys=[];
					...
					if(checkboxkeys.length==0){
						...
						if(confirmflag){
							$.ajax({
								type:"post",
								url:"${ctx}/sonActivityPrizeCfg/delete.${urlExt}",
				...
				$("#sonActivityPrizeCfg_add").click(function(){
					thickbox.attr("title", "添加学校");
					thickbox.attr("href","${ctx}/sonActivityPrizeCfg/enterAdd.${urlExt}?width=650&height=280");
					thickbox.click();
				});
				//查看
				$("#sonActivityPrizeCfg_view").click(function(){
					var checkboxkeys=[];
					var selectedUsers = grid.getSelectedItems();
					var selectIndex = '';
					for(var index in selectedUsers){
					...
					if(checkboxkeys.length==0){
						...
						thickbox.attr("href","${ctx}/sonActivityPrizeCfg/view.${urlExt}?dataid="+checkboxkeys.toString()+"&width=600&height=200");
						thickbox.click();
					}
				});
				//修改
				$("#sonActivityPrizeCfg_edit").click(function(){
					var checkboxkeys=[];
					var selectedUsers = grid.getSelectedItems();
					var selectIndex = '';
					for(var index in selectedUsers){
					...
					if(checkboxkeys.length==0){
						...
						var title = "修改信息";
						thickbox.attr("title", title);
						thickbox.attr("href","${ctx}/sonActivityPrizeCfg/enterEdit.${urlExt}?dataid="+checkboxkeys.toString()+"&width=600&height=320");
						thickbox.click();
					}
				});
    			...
    			function changeToolbarState() {
					...
					$("#sonActivityPrizeCfg_view").attr("disabled", !(hasSelected && (!selectedMany)));
					$("#sonActivityPrizeCfg_edit").attr("disabled", !(hasSelected && (!selectedMany)));
					$("#sonActivityPrizeCfg_del").attr("disabled", !(hasSelected && (!selectedMany)));
		    		}
	...
	<div class="main" id="main">
		...
		<div class="toolbar">
			<li><input type="button" id="sonActivityPrizeCfg_view" disabled="disabled" class="view" style="display: none" value="查看"/></li>
			<li><input type="button" id="sonActivityPrizeCfg_add" class="add" style="display: none" value="增加"/></li>
	      	<li><input type="button" id="sonActivityPrizeCfg_edit" disabled="disabled" class="edit" style="display: none" value="编辑"/></li>
	      	<li><input type="button" id="sonActivityPrizeCfg_del" disabled="disabled" class="del" style="display: none" value="删除"/></li>
  		</div>
	</div>
	
    <div id="queryD" style="width:100%;">
				<div class="query">
					<form id="searForm" method="post" action="${ctx}/sonActivityPrizeCfg/list.${urlExt}">
						<input type="hidden" name="currPage" id="currPage" value="1" />
						<input type="hidden" name="pageSize" id="pageSize" value="10" />
						<ul>
                            <li>活动：</li>
                            <li>
                                <select id="activityId" name="activityId" style="width:100px" class="searchCondition">
                            ...
                            <li>举办活动：</li>
                            <li>
                                <select id="sonActivityId" name="sonActivityId" style="width:100px" class="searchCondition">
                                    <option value="">不限</option>
								<#list SONACTIVITY?keys as key>
                                    <option value="${key}">${SONACTIVITY.get(key)}</option>
								</#list>
                                </select>
                            </li>
                            <li>奖品：</li>
                            <li>
                                <select id="prizeId" name="prizeId" style="width:100px" class="searchCondition">
                                    <option value="">不限</option>
