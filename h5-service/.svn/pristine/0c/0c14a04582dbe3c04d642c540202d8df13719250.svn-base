...
<script language="javascript" src="${wctx}/pub/script/jquery/jquery.form.js"></script>
		<script language="javascript">
			$(document).ready(function(){
				grid = new SimpleGrid({
					...
					columns : [
                        {name : "activityId", title : "所属活动", width : "80px"},
                        {name : "name", title : "名称", width : "80px"},
                        {name : "count", title : "无奖奖券", width : "80px"},
                        {name : "takeCount", title : "每人限抽次数", width : "80px"},
                        {name : "inCount", title : "每人限中次数", width : "80px"},
                        {name : "startStamp", title : "开始时间", width : "80px"},
                        {name : "endStamp", title : "截止时间", width : "80px"},
				...
				//删除
				$("#sonActivity_del").click(function(){
					var checkboxkeys=[];
					...
					if(checkboxkeys.length==0){
						...
						if(confirmflag){
							$.ajax({
								type:"post",
								url:"${ctx}/sonActivity/delete.${urlExt}",
				...
				$("#sonActivity_add").click(function(){
					thickbox.attr("title", "添加学校");
					thickbox.attr("href","${ctx}/sonActivity/enterAdd.${urlExt}?width=650&height=280");
					thickbox.click();
				});
				//查看
				$("#sonActivity_view").click(function(){
					var checkboxkeys=[];
					var selectedUsers = grid.getSelectedItems();
					var selectIndex = '';
					for(var index in selectedUsers){
					...
					if(checkboxkeys.length==0){
						...
						thickbox.attr("href","${ctx}/sonActivity/view.${urlExt}?dataid="+checkboxkeys.toString()+"&width=600&height=230");
						thickbox.click();
					}
				});
				//修改
				$("#sonActivity_edit").click(function(){
					var checkboxkeys=[];
					var selectedUsers = grid.getSelectedItems();
					var selectIndex = '';
					for(var index in selectedUsers){
					...
					if(checkboxkeys.length==0){
						...
						var title = "修改信息";
						thickbox.attr("title", title);
						thickbox.attr("href","${ctx}/sonActivity/enterEdit.${urlExt}?dataid="+checkboxkeys.toString()+"&width=600&height=320");
						thickbox.click();
					}
				});
    			...
    			function changeToolbarState() {
					...
					$("#sonActivity_view").attr("disabled", !(hasSelected && (!selectedMany)));
					$("#sonActivity_edit").attr("disabled", !(hasSelected && (!selectedMany)));
					$("#sonActivity_del").attr("disabled", !(hasSelected && (!selectedMany)));
		    		}
	...
	<div class="main" id="main">
		...
		<div class="toolbar">
			<li><input type="button" id="sonActivity_view" disabled="disabled" class="view" style="display: none" value="查看"/></li>
			<li><input type="button" id="sonActivity_add" class="add" style="display: none" value="增加"/></li>
	      	<li><input type="button" id="sonActivity_edit" disabled="disabled" class="edit" style="display: none" value="编辑"/></li>
	      	<li><input type="button" id="sonActivity_del" disabled="disabled" class="del" style="display: none" value="删除"/></li>
  		</div>
	</div>
	
    <div id="queryD" style="width:100%;">
				<div class="query">
					<form id="searForm" method="post" action="${ctx}/sonActivity/list.${urlExt}">
